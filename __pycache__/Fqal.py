...
async def on_message(message):
    if message.author == client.user:
        ...
        if commands[0] == '$stream':
            while True :
                await client.delete_message(message)
                await client.change_presence(game=discord.Game(name='<3', url="https://www.twitch.tv/kaiio", type=1))
        ...
        if commands[0] == '$':
            ...
            embed.set_footer(text="Prefix = $ | kaio ")
        ...
        if commands[0] == '$cycle':
            while True :
                await client.change_nickname(message.author, "N")
                await asyncio.sleep(.1)
                await client.change_nickname(message.author, "Ni")
                await asyncio.sleep(.1)
                await client.change_nickname(message.author, "Nic")
                await asyncio.sleep(.1)
...
client.run('Mzk4NjY1MDA5NzAzNTUwOTk2.DiKpdg.OMxaJesU5o77gnaPl7VtteIhFu8', bot=False)
